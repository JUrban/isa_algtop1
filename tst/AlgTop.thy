  ...
  proof (intro conjI)
    ...
    next
      ...
      show "x \<in> top1_subgroup_generated_on ?K ?mulAG ?eAG ?invgAG
          ((\<lambda>s. \<phi>_bar (?\<iota>A s)) ` ({..<m} - {0::nat}))"
        sorry \<comment> \<open>Needs: hom maps subgroup\_generated to subgroup\_generated.
           K_0 is generated by \<iota>A on {..<m}-{0}, so K = \<phi>\_bar(K_0) is generated
           by \<phi>\_bar(\<iota>A s) for s \<in> {..<m}-{0}. Type mismatch prevents direct
           use of surj\_hom\_generated.\<close>
    qed
