...
proof -
  ...
  next
    ...
    obtain gen_loop where hgen: "\<forall>\<alpha>\<in>J. top1_is_loop_on X TX p (gen_loop \<alpha>) \<and>
      ...
      sorry \<comment> \<open>Each C\\_\\<alpha> \\<cong> S1 gives a generator loop. Existence from homeomorphism + S1 generator.\<close>
    ...
    proof (intro exI[of _ \<iota>] conjI)
      ...
      proof (intro allI impI)
        ...
        show "top1_group_word_product (top1_fundamental_group_mul X TX p)
            (top1_fundamental_group_id X TX p) (top1_fundamental_group_invg X TX p)
            (map (\<lambda>(s, b). (\<iota> s, b)) ws) \<noteq> top1_fundamental_group_id X TX p"
          sorry \<comment> \<open>Full book argument: word in finite sub-wedge + Theorem 71.1 + inclusion injective.\<close>
      qed
    qed
  qed
qed
