...
proof -
  ...
  \<comment> \<open>For each pair (s1,s2): from hcomm, \<exists>w \<in> R with w = commutator(s1',s2').
     hgens\_commute gives \<pi>(\<iota>(s1')) and \<pi>(\<iota>(s2')) commute.
     For the torus: S={0,1}, single relator for pair (0,1).
     So all generators commute. Since generators generate G, G abelian.
     This is a standard group theory fact; sorry for now.\<close>
