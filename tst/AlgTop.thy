...
proof -
  ...
  \<comment> \<open>Munkres: f0 is a loop in Int B2 - {0} based at q representing a generator.
     Then \<bar>\<gamma> * (f0 * \<gamma>) is path-homotopic in B2 - {0} to f (standard S1 loop
     based at p). So \<hat>\<delta>([g0]) = [g] where g = h \<circ> f = k \<circ> p.\<close>
  have hgenerator_maps_to_kp:
    "\<exists>gen0. gen0 \<in> top1_fundamental_group_carrier ?UV ?TUV ?b
      \<and> top1_fundamental_group_induced_on ?U ?TU a A ?TA a (\<lambda>x. x)
         ...
         = top1_fundamental_group_induced_on top1_S1 top1_S1_topology (1, 0)
             ...
             {g. top1_loop_equiv_on top1_S1 top1_S1_topology (1, 0)
                   (\<lambda>s. (cos (2 * pi * s), sin (2 * pi * s))) g}"
  proof -
    \<comment> \<open>Step 6a: UV = V - {x0} \<cong> Int B2 - {0} has infinite cyclic \<pi>_1.\<close>
    \<comment> \<open>The standard loop f(s)=(cos 2\<pi>s, sin 2\<pi>s) generates \<pi>_1(B2-{0}, p=(1,0)).\<close>
    \<comment> \<open>Applying h: g = h \<circ> f is a loop in X based at a = h(1,0).\<close>
    \<comment> \<open>g = h \<circ> f = k \<circ> p (since h|_{S1} = k and f = p on S1).\<close>
    let ?f = "\<lambda>s::real. (cos (2*pi*s), sin (2*pi*s))"
    let ?g = "\<lambda>s. h (?f s)"
    have hg_eq_kp: "?g = ?\<iota> \<circ> ?f"
      by (rule ext) (by100 simp)
    \<comment> \<open>Step 6b: \<pi>_1(UV, b) is infinite cyclic, generated by [g0] where g0 = h \<circ> f0.
       f0 is a loop in Int B2 - {0} based at q = (1/2, 0).\<close>
    ...
    \<comment> \<open>Witness: gen0 = [h \<circ> f0] where f0(s) = (1/2)(cos 2\<pi>s, sin 2\<pi>s).
       g0 = h \<circ> f0 is loop in UV at b. Key homotopy: \<bar>\<delta>*(g0*\<delta>) \<simeq> g in U
       (from \<bar>\<gamma>*(f0*\<gamma>) \<simeq> f in B2-{0}, both generators). Image chain = [\<iota>\<circ>f] = [k\<circ>p].\<close>
    show ?thesis sorry
  qed
  ...
  have hiso: "top1_groups_isomorphic_on
        ...
        (top1_quotient_group_mul_on
           (top1_fundamental_group_mul A (subspace_topology X TX A) a))"
    sorry \<comment> \<open>Combine: SvK at b + base-point change + deformation retract + generator tracking.\<close>
