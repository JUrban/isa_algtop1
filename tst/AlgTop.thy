...
proof -
  ...
  proof (intro conjI)
    ...
    show "?K = top1_subgroup_generated_on ?K ?mulAG ?eAG ?invgAG
        ((\<lambda>s. \<phi>_bar (?\<iota>A s)) ` ({..<m} - {0::nat}))"
      sorry \<comment> \<open>K = \<phi>\_bar(K_0), K_0 = subgroup\_generated(\<iota>A ` ({..<m}-{0})),
         \<phi>\_bar is a hom, so \<phi>\_bar(generated(S)) = generated(\<phi>\_bar(S)) when restricted.\<close>
    ...
    show "\<forall>c. finite {s \<in> {..<m} - {0::nat}. c s \<noteq> 0} \<longrightarrow>
      ...
      sorry \<comment> \<open>Independence: use difference-coordinate homs \<delta>_j per expert audit 11.
         \<delta>_j kills N\_AbelF (since \<delta>_j(\<beta>)=0), descends to quotient AbelG.
         If integer combination of \<phi>\_bar(\<iota>A(s)) for s>0 = eAG, then for each j,
         applying \<delta>_j recovers the j-th coefficient = 0.\<close>
  qed
  ...
  proof (intro ballI)
    ...
    show "\<exists>k\<in>?K. \<exists>t\<in>top1_torsion_subgroup_on ?AbelG ?mulAG ?eAG. h = ?mulAG k t"
      sorry \<comment> \<open>Generator-level decomposition + subgroup generation.
         Uses: h\<phi>\_surj, generators of AbelG = {\<phi>\_bar(\<iota>A i)},
         \<phi>\_bar(\<iota>A i) \<in> K for i>0, \<phi>\_bar(\<iota>A 0) \<in> K\<cdot>{eAG,\<beta>G}.\<close>
  qed
