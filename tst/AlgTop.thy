...
\<comment> \<open>SORRY ANALYSIS (as of 2026-06-15, session 1523):

BREAKTHROUGH (session 1522): front\\_cancel\\_proper proved via uncancel+uniqueness.
  The 6000-line broken tau-map proof was replaced by a 30-line proof:
  scheme\\_quotient\\_exists(w) + uncancel\\_proved + scheme\\_quotient\\_uniqueness + transfer.
  File shrank from ~9000 to ~2960 lines. Build: 5-6 seconds.

FALSE LEMMAS (kept as sorry for dependency tracking):
- quotient\\_of\\_scheme\\_edge\\_permutation (line 122): arbitrary permutation \\<noteq> quotient.
- h\\_tau\\_inj: DELETED (was inside the removed tau-map proof).

REMAINING ~30 SORRY TACTICS (grouped by category):

A. CUT-PASTE SAME-SPACE (3 sorrys: lines 145, 157, 185):
   quotient\\_of\\_scheme\\_cut\\_paste/2/opp. Need multi-polygon infrastructure
   or polygon rearrangement homeomorphism (polygon\\_sub\\_rearrange\\_homeo at line 1075).

B. GENERAL CANCEL/UNCANCEL (2 sorrys: lines 214, 225):
   front\\_cancel\\_realization\\_homeo + quotient\\_of\\_scheme\\_uncancel\\_front.
   General (non-proper) cases. Proper cases are PROVED.
   Non-fresh uncancel is genuinely FALSE.

C. VERTEX UNIQUENESS (4 sorrys: lines 669, 676, 814, 928):
   In scheme\\_quotient\\_uniqueness. Need C12 (vertex-edge separation)
   which is NOT in the definition of top1\\_quotient\\_of\\_scheme\\_on.
   NOT on critical path (both quotients on critical path come from
   scheme\\_quotient\\_exists which provides C12).

D. POLYGON REARRANGEMENT (3 sorrys: lines 1075, 1185, 1234):
   polygon\\_sub\\_rearrange\\_homeo + two cut-reglue homeo (proper variants).
   Needed for cut-paste proper operations.

E. CONTEXT-LEFT (10 sorrys: lines 1561-1719):
   Most are NOT on critical path (classification chain only uses
   v\\_context\\_left with inner v\\_relabel(fresh), which is PROVED).

F. GENUINELY FALSE (2 sorrys: lines 1444, 1611):
   length(u@v) < 3 after cancel. Need precondition fix in cached session.

G. MISC (3 sorrys: lines 1477, 1484, 1786):
   Edge-permutation references (FALSE) + Theorem\\_76 relabel (freshness).

H. FINAL ASSEMBLY (3 sorrys: lines 2734, 2775, 2817):
   Thm 78.2 induction, scheme extraction, sphere realization.

CRITICAL PATH for classification theorem:
scheme\\_normal\\_form\\_valid (PROVED, cached)
+ valid\\_equiv\\_preserves\\_quotient\\_homeo
  needs: v\\_cancel (sorry 214), v\\_cut\\_paste/2/opp (sorrys 145/157/185),
         v\\_cancel\\_reverse (depends on uncancel\\_front sorry 225)
+ Thm 78.2 (sorry 2734) + scheme extraction (sorry 2775)
+ sphere realization (sorry 2817)

KEY PROVED INFRASTRUCTURE:
- scheme\\_quotient\\_exists (ZERO SORRY, with C1-C12 + vtgt)
- scheme\\_quotient\\_uniqueness (modulo vertex sorrys C-D above)
- front\\_cancel\\_proper (PROVED via uncancel+uniqueness, for proper+fresh)
- quotient\\_of\\_scheme\\_uncancel\\_front\\_proper (PROVED)
- quotient\\_of\\_scheme\\_uncancel\\_proved (depends on uncancel\\_front sorry)
- scheme\\_quotient\\_transfer\\_along\\_homeomorphism
- scheme\\_normal\\_form\\_valid (PROVED, cached)\<close>
...
\<comment> \<open>Munkres Theorem 76.1 and multi-polygon quotients.
   ...
   Step 2 requires genuine multi-polygon reasoning (suffix operations).
   Therefore: the cut-paste lemmas below are sorry'd pending multi-polygon infrastructure.

   For CANCEL specifically: front\\_cancel\\_proper is proved via the
   uncancel + uniqueness shortcut (no multi-polygon needed).
   For CUT-PASTE: multi-polygon infrastructure IS needed.\<close>

\<comment> \<open>Two-polygon quotient (Munkres §76, following Theorem 76.1).
   Given two disjoint convex polygons P1, P2 with labelling schemes w1, w2,
   the two-polygon quotient X is obtained by identifying all matching-label edges
   across both polygons. This includes:
   - Within-P1 identifications (from labels appearing twice in w1)
   - Within-P2 identifications (from labels appearing twice in w2)
   - Cross-polygon identifications (from labels appearing in both w1 and w2)

   For Theorem 76.1: w1 = y0 @ [c^{-1}], w2 = [c] @ y1, where c connects the
   two polygons. The cross-polygon c-identification pastes P1 and P2 together.\<close>
...
\<comment> \<open>Suffix rotate via separator (per-polygon rotation from Munkres §76).
   If c is a fresh separator label, rotating the suffix after c preserves the quotient.
   This corresponds to rotating one polygon independently in the multi-polygon view.

   Book justification (§76 operation (iv)): "Permute. One can replace any one of the
   schemes wi by a cyclic permutation of wi. This amounts to renumbering the vertices
   of the polygonal region Pi so as to begin with a different vertex; it does not affect
   the resulting quotient space."

   In single-polygon terms: y0 @ [c^{-1}, c] @ y1 has the same quotient as
   y0 @ [c^{-1}, c] @ rotate(y1), because:
   1. The scheme y0 @ [c^{-1}, c] @ y1 represents multi(y0 c^{-1}, c y1)
   2. Rotating P2 (scheme c y1) to get c y1' doesn't change X (book operation (iv))
   3. The multi-scheme (y0 c^{-1}, c y1') represents y0 @ [c^{-1}, c] @ y1'

   The proof needs: the geometric fact that renumbering vertices of a polygon
   (starting from a different vertex) doesn't change the quotient space.
   This is because the quotient depends only on the identification pattern,
   and cyclic rotation just renumbers vertices.\<close>
...
\<comment> \<open>Reformulation: the key lemma is at the POLYGON level, not the disk level.
   Given a convex polygon P with n vertices and the convex hull definition,
   construct a homeomorphism \\<phi>: P \\<to> P that cyclically shifts edges within
   one sub-polygon (bounded by a diagonal) while fixing the other sub-polygon.

   The construction:
   1. Sub-polygon Q1 = convex hull of vertices cut\\_a, cut\\_a+1, ..., cut\\_b (mod n)
   2. Sub-polygon Q2 = convex hull of vertices cut\\_b, cut\\_b+1, ..., cut\\_a (mod n)
   3. Let c1 = centroid of Q1
   4. For each ray from c1 to a boundary point b of Q1:
      - If b is on the diagonal (segment v\\_{cut\\_a} v\\_{cut\\_b}): \\<phi>(b) = b
      - If b is on edge k (where k is between cut\\_a and cut\\_b-1):
        map to corresponding point on edge (k + shift) mod (cut\\_b - cut\\_a)
   5. Extend radially: \\<phi>((1-\\<lambda>)*c1 + \\<lambda>*b) = (1-\\<lambda>)*c1 + \\<lambda>*\\<phi>(b)
   6. On Q2: \\<phi> = identity

   Continuity: at the diagonal (shared boundary), both sides give identity.
   At edges within Q1: the cyclic shift preserves adjacency
     (consecutive edges remain consecutive).
   Bijectivity: cone extension from centroid is bijective on each sub-polygon.\<close>

lemma polygon_sub_rearrange_homeo:
  fixes vx vy :: "nat \<Rightarrow> real" and n :: nat
    and cut_a cut_b :: nat and shift :: nat
    and P :: "(real \<times> real) set"
  assumes hn: "n \<ge> 3"
      and hvx: "\<forall>i<n. \<forall>j<n. i \<noteq> j \<longrightarrow> (vx i, vy i) \<noteq> (vx j, vy j)"
      and hP_strict: "\<forall>i<n. \<forall>k<n. k \<noteq> i \<longrightarrow> k \<noteq> Suc i mod n \<longrightarrow>
          (vx k - vx i) * (vy (Suc i mod n) - vy i) -
          (vy k - vy i) * (vx (Suc i mod n) - vx i) < 0"
      and hcut_a: "cut_a < n" and hcut_b: "cut_b < n"
      and hcut_ne: "cut_a \<noteq> cut_b"
      and hm: "(cut_b - cut_a) mod n \<ge> 2"
      and hshift: "shift > 0" "shift < (cut_b - cut_a) mod n"
      and hP_def: "P = {(x, y) | x y. \<exists>coeffs. (\<forall>i<n. coeffs i \<ge> 0)
                   \<and> (\<Sum>i<n. coeffs i) = 1
                   \<and> x = (\<Sum>i<n. coeffs i * vx i)
                   \<and> y = (\<Sum>i<n. coeffs i * vy i)}"
  shows "\<exists>\<phi>. bij_betw \<phi> P P
    \<and> continuous_on P \<phi>
    \<and> continuous_on P (inv_into P \<phi>)
    \<and> (\<forall>k<n. \<forall>t\<in>I_set.
         \<phi> ((1-t)*vx k + t*vx(Suc k mod n), (1-t)*vy k + t*vy(Suc k mod n))
       = (let m = (cut_b - cut_a) mod n;
              new_k = (if (k - cut_a) mod n < m
...
\<comment> \<open>ALTERNATIVE APPROACH for front\\_cancel\\_proper (to replace broken \\<tau>-map approach):
   The \\<tau>-map approach fails because h\\_tau\\_inj is PROVABLY FALSE.

   APPROACH: Quotient factorization + scar collapse (avoids multi-polygon quotients).
   Given: Y \\<cong> P\\_e / R\\_e where R\\_e = R\\_spur \\<union> R\\_{y0y1}.

   Step A: Factor the quotient.
     R\\_e is generated by two types of identification:
     (1) R\\_spur: edges 0 \\<leftrightarrow> 1 (spur a \\<leftrightarrow> a\\<inverse>, opposite orientation)
     (2) R\\_{y0y1}: edges 2..n+1 identified by scheme w
     Since label a is fresh, the spur identification R\\_spur commutes with R\\_{y0y1}.
     So P\\_e / R\\_e = (P\\_e / R\\_spur) / R\\_{y0y1}'.

   Step B: Scar collapse.
     R := P\\_e / R\\_spur is the polygon with the two spur edges identified.
     R has a "scar" (1D arc from junction vertex to spur tip).
     Collapsing the scar gives R \\<cong> n-gon P\\_m.
     The key fact: collapsing a free arc from boundary to interior
     in a 2-manifold gives a homeomorphism. This is the "slit closure"
     theorem (reverse of cutting a slit from boundary into interior).

   Step C: Transfer.
     Y \\<cong> (P\\_e / R\\_spur) / R\\_{y0y1}' \\<cong> P\\_m / R\\_{y0y1} \\<cong> Y\\_w.
     The last step uses scheme\\_quotient\\_uniqueness.

   DIFFICULTY: Steps A and B need formal infrastructure not yet in the formalization:
   - Quotient factorization (quotient of quotient = quotient by join)
   - Scar collapse homeomorphism (arc-from-boundary collapsing)
   - Boundary preservation under the homeomorphism (for scheme identification transfer)

   These are all CORRECT mathematically; the question is formalization effort.\<close>
...
\<comment> \<open>OLD PROOF (6000+ lines) REMOVED.
   ...
   h\\_tau\\_inj is PROVABLY FALSE (see sorry analysis at top of file).
   The new proof above uses scheme\\_quotient\\_exists + uncancel + uniqueness.
   It depends on uncancel\\_front (sorry) and scheme\\_quotient\\_uniqueness vertex sorrys,
   which are NOT false (just unproved). This is strictly better than depending on h\\_tau\\_inj.\<close>

\<comment> \<open>RETAINED from old proof: the following infrastructure lemmas may be useful.
