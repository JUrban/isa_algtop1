...
next
  ...
  proof (cases "v = []")
    ...
    have ha_fresh_u2: "a \<notin> fst ` set u2"
      sorry \<comment> \<open>Needs: a \\<notin> fst ` set u2. True for proper schemes where a appears only at positions 0,k.\<close>
        ...
        proof (intro ballI impI allI)
          fix p p' assume hp: "p \<in> P2" and hp': "p' \<in> P2"
              and hint: "\<forall>i<length ?w'. \<forall>t\<in>I_set.
                p \<noteq> ((1-t)*vx2 i+t*vx2(Suc i mod length ?w'),(1-t)*vy2 i+t*vy2(Suc i mod length ?w'))"
              and heq: "g p = g p'"
          \<comment> \<open>C8 proof sketch: g is injective on target polygon interior.
             p is not on any TARGET boundary edge, so p is a target interior point.
             Two cases by cross\\_diag sign:
             Case 1: cross\\_diag(p) < 0 (strict left). g(p) = q2(phi\\_L(p)).
               phi\\_L(p) is in the old polygon interior (left sub-polygon interior).
               Similarly for p': either same half or different half.
               If same half: q2(phi\\_L(p)) = q2(phi\\_L(p')) with both in old interior
                 => phi\\_L(p) = phi\\_L(p') by old C8 => p = p' by phi\\_L injectivity.
               If different half: q2(left interior) \\<inter> q2(right interior) = \\<emptyset> by old C8.
             Case 2: cross\\_diag(p) > 0 (strict right). Symmetric.
             Case 3: cross\\_diag(p) = 0 (on virtual diagonal).
          ...
          have hphi_L_diagonal_on_edge: "\<And>p. p \<in> P2 \<Longrightarrow> cross_diag p = 0 \<Longrightarrow>
              (\<forall>i<?n. \<forall>t\<in>I_set.
                p \<noteq> ((1-t)*vx2 i + t*vx2(Suc i mod ?n), (1-t)*vy2 i + t*vy2(Suc i mod ?n))) \<Longrightarrow>
              \<exists>t\<in>{0<..<(1::real)}. phi_L p = ((1-t)*vx2 0 + t*vx2(Suc 0 mod ?n),
                                               (1-t)*vy2 0 + t*vy2(Suc 0 mod ?n))"
            sorry \<comment> \<open>Diagonal \\<to> old edge 0 via LEAST=k-1 + Cramer s=0,tp=t.\<close>
              ...
              proof (cases "cross_diag p > 0")
                ...
                next
                  ...
                  next
                    ...
                    moreover have "phi_L p' \<in> P2" using hphi_L_in_P2[OF hp'] .
                    ultimately have "phi_R p = phi_L p'" using hC8_at_p by (by100 blast)
                    \<comment> \<open>phi\\_L(p') is on old edge (diagonal maps to edge 0), phi\\_R(p) is old interior.
                       Contradiction with phi\\_R(p) = phi\\_L(p').\<close>
                    show ?thesis sorry \<comment> \<open>phi\\_L(diagonal) on old edge vs phi\\_R interior.\<close>
                  qed
                qed
              next
                case False2: False
                ...
                proof (cases "cross_diag p' > 0")
                  ...
                  \<comment> \<open>phi\\_L(p) on old edge (diagonal), phi\\_R(p') old interior.
                     C8 at phi\\_R(p') gives phi\\_R(p')=phi\\_L(p). Contradicts phi\\_L(diagonal) on edge.\<close>
                  show ?thesis sorry \<comment> \<open>Diagonal vs right-half: phi\\_L(diagonal) on edge, phi\\_R(interior) not.\<close>
                next
                  case False3: False
                  ...
                  hence "q2 (phi_L p) = q2 (phi_L p')" using hgp hg_eq by simp
                  moreover have "phi_L p' \<in> P2" using hphi_L_in_P2[OF hp'] .
                  \<comment> \<open>Need hC8 at phi\\_L(p): phi\\_L(p) is on old boundary if p is on diagonal.
                     But old C8 requires phi\\_L(p) NOT on boundary. So this case needs C9 instead.\<close>
                  \<comment> \<open>Alternative: use phi\\_L injectivity directly if phi\\_L is injective on left+diagonal.\<close>
                  ultimately have "phi_L p = phi_L p'"
                    sorry \<comment> \<open>Need: either C8 at phi\\_L(p) (requires phi\\_L(p) not on edge) or injectivity.\<close>
