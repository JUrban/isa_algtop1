...
text \<open>Group presentation: G is presented by generators S modulo relations R,
  ...
  and we express the property as: there exist a free group F on S, a surjective
  hom \<pi>: F \<rightarrow> G whose kernel is the normal subgroup of F generated by the image
  of the relator words under the canonical \<iota>_list : 's list \<rightarrow> F.\<close>
     ...
     (\<exists>(F::'g set) mulF eF invgF \<iota> \<pi>.
      ...
      \<and> top1_group_kernel_on F e \<pi>
           = top1_normal_subgroup_generated_on F mulF eF invgF
               {r. \<exists>w\<in>R. r = foldr mulF (map \<iota> w) eF})"
     \<comment> \<open>The kernel of \<pi> equals the normal-closure of the relator images in F.\<close>
...
definition top1_subgroup_has_index_on ::
  ...
  "top1_subgroup_has_index_on G mul H k \<longleftrightarrow>
     card (top1_quotient_group_carrier_on G mul H) = k"
