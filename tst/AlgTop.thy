...
theorem Theorem_68_7_quotient_free_product:
  assumes "top1_is_group_on (G1::'g set) mul1 e1 invg1"
      and "top1_is_group_on (G2::'g set) mul2 e2 invg2"
      and "top1_normal_subgroup_on G1 mul1 e1 invg1 N1"
      and "top1_normal_subgroup_on G2 mul2 e2 invg2 N2"
  shows "True"  \<comment> \<open>(G1*G2)/\<langle>\<langle>N1\<union>N2\<rangle>\<rangle> \<cong> (G1/N1)*(G2/N2) — requires quotient-group machinery\<close>
  by simp
...
(** from \<S>71 Theorem 71.1: finite wedge of circles has free fundamental group
    ...
    Statement: there exists a group (G, mul, e, invg) that is free on n generators
    and whose carrier bijects with \<pi>_1(wedge, p). **)
theorem Theorem_71_1_wedge_of_circles_finite:
  fixes n :: nat and X :: "'a set" and TX :: "'a set set" and p :: 'a
  assumes "True"  \<comment> \<open>X is a wedge of n circles at common point p\<close>
...
theorem Theorem_71_3_wedge_of_circles_general:
  ...
  assumes "True"  \<comment> \<open>X is a wedge of circles {S_\<alpha>}_{\<alpha>\<in>J} at common point p\<close>
...
(** from \<S>72 Theorem 72.1: attaching a 2-cell kills the homotopy class of
    the attaching map. \<pi>_1(X) is isomorphic to \<pi>_1(A)/N where N is normal
    closure of the attaching-map loop class. **)
theorem Theorem_72_1_attaching_two_cell:
  ...
  assumes "is_topology_on_strict X TX"
      ...
      and "True"  \<comment> \<open>h : B^2 \<rightarrow> X with h|Int B^2 bijective onto X-A, h(S^1) \<subseteq> A\<close>
  shows "\<exists>\<phi>. True"  \<comment> \<open>\<pi>_1(X, a) \<cong> \<pi>_1(A, a) / normal-closure([\<partial>B^2])\<close>
  by simp
...
theorem Theorem_73_1_torus_presentation:
  ...
  assumes "True"  \<comment> \<open>T_torus is the torus S^1 \<times> S^1\<close>
  shows "\<exists>\<phi>. bij_betw \<phi>
...
theorem Theorem_73_4_dunce_cap:
  ...
  assumes "n > 0"
      and "True"  \<comment> \<open>X is the n-fold dunce cap\<close>
  shows "\<exists>\<phi>. bij_betw \<phi>
          (top1_fundamental_group_carrier X TX x0)
          (top1_Zn_group n)"
  sorry
