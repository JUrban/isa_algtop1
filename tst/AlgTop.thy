...
lemma R2_minus_point_path_connected:
  "p \<in> (UNIV :: (real \<times> real) set) \<Longrightarrow>
   top1_path_connected_on (UNIV - {p})
     (subspace_topology UNIV (product_topology_on top1_open_sets top1_open_sets) (UNIV - {p}))"
  \<comment> \<open>R^2 - {p} is path-connected: given x, y \<noteq> p, connect by going around p
     if the straight line passes through p, otherwise use the straight line.\<close>
  sorry
...
lemma Lemma_61_2_nulhomotopy_textbook:
  ...
  shows "top1_nulhomotopic_on A TA
           (top1_S2 - {a, b}) (subspace_topology top1_S2 top1_S2_topology (top1_S2 - {a, b})) f"
  \<comment> \<open>Proof via stereographic projection: replace S^2 by R^2 \<union> {\<infinity>}, a \<mapsto> 0, b \<mapsto> \<infinity>.
     Then f: A \<rightarrow> R^2 - {0}, and 0 lies in unbounded component of R^2 - f(A).
     Translation homotopy G(x,t) = f(x) - \<alpha>(t) stays in R^2 - {0},
     then straight-line homotopy H(x,t) = t*f(x) - p contracts to constant.\<close>
  sorry
...
proof -
  ...
  proof -
    ...
    qed
    have hg_cont: "top1_continuous_map_on I_set I_top top1_S1 top1_S1_topology g" sorry
    \<comment> \<open>f \<circ> g: [0,1] \<rightarrow> f(upper) = A1 is injective continuous.\<close>
    have hfg_cont: "top1_continuous_map_on I_set I_top X TX (f \<circ> g)" sorry
  ...
  qed
  have hA2_arc: "top1_is_arc_on ?A2 (subspace_topology X TX ?A2)" sorry
  show ?thesis
...
proof
  ...
  have hS2_2pts_pc: "top1_path_connected_on (top1_S2 - {f(1,0), f(-1,0)})
    ...
    sorry \<comment> \<open>S^2 - 2pts \<cong> R^2 - {pt}: use S2_minus_point_homeo_R2 + restrict.\<close>
...
proof (rule ccontr)
  ...
  proof -
    ...
    have hU_nul: "\<forall>g. top1_is_loop_on ?X ?TX x0 g \<and> g ` I_set \<subseteq> ?U \<longrightarrow>
        top1_path_homotopic_on ?X ?TX x0 x0 g (top1_constant_path x0)" sorry
    ...
    show ?thesis unfolding top1_simply_connected_on_def sorry
  qed
