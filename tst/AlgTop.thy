...
proof -
  ...
  have hpi_free: "top1_is_free_group_full_on ?pi ?mul ?eid ?invg
    ...
    sorry \<comment> \<open>Free group on {0}:
       (1) \\<pi>\\_1 group \\<checkmark> (hpi\\_grp)
       (2) \\<iota>(0) = [\\<alpha>*\\<beta>] \\<in> \\<pi>\\_1 \\<checkmark> (h\\_gen\\_in\\_pi)
       (3) inj\\_on \\<iota> {0} \\<checkmark> (trivial for singleton)
       (4) \\<pi>\\_1 = generated by {[\\<alpha>*\\<beta>]} — from hgen
       (5) reduced word \\<noteq> e — from hinf\\_order
       Needs hgen as prerequisite.\<close>
