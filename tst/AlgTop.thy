...
proof
  ...
  have hcard1: "top1_fundamental_group_carrier top1_S1 top1_S1_topology (1,0)
    ...
    sorry \<comment> \<open>sc \<Rightarrow> all loops equivalent to const \<Rightarrow> 1 class.\<close>
  \<comment> \<open>But bij_betw to UNIV::int (which has \<ge> 2 elements) requires \<ge> 2 elements.\<close>
  \<comment> \<open>bij_betw \<phi> (carrier) (UNIV::int set) with card(carrier) = 1 but UNIV::int infinite.\<close>
  show False sorry
qed
...
proof
  ...
  \<comment> \<open>Compact S^1 to Hausdorff S^2: continuous bijection = homeomorphism.\<close>
  \<comment> \<open>But S^1 not simply connected while S^2 is (via S2_minus_point_sc). Contradiction.\<close>
  show False sorry
qed
