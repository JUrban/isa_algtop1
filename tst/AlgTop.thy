...
lemma quotient_of_scheme_cancel_front:
  ...
  shows "top1_quotient_of_scheme_on Y TY w"
  sorry \<comment> \<open>§76(vi): Fold front cancel pair. Geometric polygon construction.
     Plan: extract vx/vy from old quotient, define new polygon P' by
     skipping vertex 1: vx'(i)=vx(i+2), vy'(i)=vy(i+2).
     P' is convex hull of new vertices (subset of convex polygon is convex).
     Verify all quotient conditions with index shift i -> i+2.
     Edge i of P' = edge i+2 of P, so scheme for P' = w (positions 2..n-1).\<close>
