...
proof -
  ...
  next
    ...
    have hgens: "\<forall>\<alpha>\<in>J. \<exists>f\<alpha>. top1_is_loop_on X TX p f\<alpha> \<and>
        f\<alpha> ` top1_unit_interval \<subseteq> C \<alpha> \<and>
        (\<forall>g. top1_is_loop_on (C \<alpha>) (subspace_topology X TX (C \<alpha>)) p g \<longrightarrow>
          (\<exists>n::int. top1_path_homotopic_on X TX p p g
            (top1_path_power f\<alpha> p (nat (abs n)))))"
      sorry \<comment> \<open>Each C\\_\\<alpha> \\<cong> S1 has \\<pi>\\_1 \\<cong> Z, generated by some loop.\<close>
    \<comment> \<open>Step 2: define \\<iota>.\<close>
    obtain gen_loop where hgen: "\<forall>\<alpha>\<in>J. top1_is_loop_on X TX p (gen_loop \<alpha>) \<and>
        (gen_loop \<alpha>) ` top1_unit_interval \<subseteq> C \<alpha>"
      sorry \<comment> \<open>From hgens via choice.\<close>
    ...
    proof (intro exI[of _ \<iota>] conjI)
      ...
      show "inj_on \<iota> J"
        sorry \<comment> \<open>Book: i\\_\\<beta> is a monomorphism. If \\<iota>(\\<alpha>) = \\<iota>(\\<beta>) then gen\\_loop \\<alpha> \\<sim> gen\\_loop \\<beta>.
           ...
           unless \\<alpha> = \\<beta> (since C\\_\\<alpha> \\<inter> C\\_\\<beta> = {p}).\<close>
      show "top1_fundamental_group_carrier X TX p =
        ...
        sorry \<comment> \<open>Book: {G\\_\\<alpha>} generate \\<pi>\\_1. Any loop f lies in a finite sub-wedge,
           where Theorem 71.1 gives [f] as a product of generators.\<close>
      show "\<forall>ws::('i \<times> bool) list.
        ...
        sorry \<comment> \<open>Book: suppose reduced word w = id. The word uses finitely many generators.
           The corresponding loop lies in a finite sub-wedge. By hhtpy\\_finite,
           the null-homotopy also lies in a finite sub-wedge. By Theorem 71.1,
           the word is non-trivial in the sub-wedge \\<Rightarrow> contradiction.\<close>
    qed
  qed
qed
