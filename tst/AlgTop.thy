...
proof -
  ...
  proof (intro conjI hTS ballI)
    ...
    obtain f where hf_path: "top1_is_path_on UNIV (product_topology_on top1_open_sets top1_open_sets) x y f"
        and hf_avoid: "\<forall>t\<in>I_set. f t \<noteq> p"
      sorry \<comment> \<open>Path x\<rightarrow>y avoiding p. Use straight line when not collinear with p,
             perpendicular detour when collinear (both segments then avoid p).\<close>
...
proof -
  ...
  have hC_bounded: "\<exists>M. \<forall>p \<in> h ` (C - {b}). fst p ^ 2 + snd p ^ 2 \<le> M"
    sorry \<comment> \<open>h(C) compact (continuous image of compact), hence bounded in R^2.\<close>
  show ?thesis sorry
qed
...
proof -
  ...
  have hg_cont: "top1_continuous_map_on A TA (UNIV - {?ha})
      ...
      (h \<circ> f)" sorry
  \<comment> \<open>Step 3: Since a, b in same component of S^2-f(A), and h maps this homeomorphically,
     h(a) is in the unbounded component of R^2-g(A).\<close>
  \<comment> \<open>Step 4: g(A) = (h\<circ>f)(A) is compact (continuous image of compact A), hence bounded.
     Choose M with g(A) \<subseteq> ball(0,M). Choose p outside ball(0,M).
     Then h(a) and p are in the same (unbounded) component of R^2-g(A).\<close>
  \<comment> \<open>Step 5: Path \<alpha> from h(a) to p in R^2-g(A) (same component, locally path-connected).\<close>
  \<comment> \<open>Step 6: G(x,t) = g(x) - \<alpha>(t) is homotopy g \<simeq> k where k(x) = g(x) - p.
     G(x,t) \<noteq> h(a) because \<alpha>(t) \<notin> g(A) and g(x) - \<alpha>(t) = h(a) iff g(x) = \<alpha>(t) + h(a).\<close>
  \<comment> \<open>Step 7: H(x,t) = t*g(x) - p is homotopy k \<simeq> constant(-p).
     H(x,t) \<noteq> h(a) because |t*g(x)| \<le> M but |p + h(a)| > M.\<close>
  \<comment> \<open>Step 8: Transfer nulhomotopy from R^2-{ha} back to S^2-{a,b} via h^{-1}.\<close>
  show ?thesis sorry
qed
...
proof
  ...
  proof -
    ...
    next
      ...
      proof -
        have "2 * pi * t' = 2 * pi * t - 2 * pi * of_int \<lfloor>t\<rfloor>"
          unfolding t'_def by (simp add: algebra_simps)
        hence h_eq: "2 * pi * t' = 2 * pi * t - (2 * pi) * of_int \<lfloor>t\<rfloor>" by simp
        have hcos: "cos (2 * pi * t') = cos (2 * pi * t)"
        proof -
          have "cos (2 * pi * t') = cos (2 * pi * t - (2 * pi) * of_int \<lfloor>t\<rfloor>)"
            using h_eq by simp
          also have "... = cos (2 * pi * t) * cos ((2 * pi) * of_int \<lfloor>t\<rfloor>)
              + sin (2 * pi * t) * sin ((2 * pi) * of_int \<lfloor>t\<rfloor>)"
            by (rule cos_diff)
          also have "... = cos (2 * pi * t) * 1 + sin (2 * pi * t) * 0"
            by (simp add: cos_int_2pin sin_int_2pin)
          also have "... = cos (2 * pi * t)" by simp
          finally show ?thesis .
        qed
        have hsin: "sin (2 * pi * t') = sin (2 * pi * t)"
        proof -
          have "sin (2 * pi * t') = sin (2 * pi * t - (2 * pi) * of_int \<lfloor>t\<rfloor>)"
            using h_eq by simp
          also have "... = sin (2 * pi * t) * cos ((2 * pi) * of_int \<lfloor>t\<rfloor>)
              - cos (2 * pi * t) * sin ((2 * pi) * of_int \<lfloor>t\<rfloor>)"
            by (rule sin_diff)
          also have "... = sin (2 * pi * t) * 1 - cos (2 * pi * t) * 0"
            by (simp add: cos_int_2pin sin_int_2pin)
          also have "... = sin (2 * pi * t)" by simp
          finally show ?thesis .
        qed
    ...
    have "compact {0..1::real}" by (rule compact_Icc)
    hence "compact (top1_R_to_S1 ` {0..1::real})"
      using compact_continuous_image[OF hcont_S1] by simp
    hence "compact top1_S1" using hS1_img by simp
    hence "top1_compact_on top1_S1
        (subspace_topology (UNIV::(real\<times>real) set) (top1_open_sets::(real\<times>real) set set) top1_S1)"
      using top1_compact_on_subspace_UNIV_iff_compact[of top1_S1] by simp
...
proof (rule ccontr)
  ...
  proof -
    ...
    have hUV_locp: "top1_locally_path_connected_on (top1_S2 - C)
        (subspace_topology top1_S2 top1_S2_topology (top1_S2 - C))" sorry
  ...
  proof -
    ...
    proof (intro allI impI)
      ...
      have hA2_sub_comp: "A2 \<subseteq> top1_S2 - g ` I_set" using hgI_disj_A2
        using hC_sub hC_decomp by (by100 blast)
...
proof (rule ccontr)
  ...
  have hab_exists: "\<exists>a b. a \<in> top1_S2 - D \<and> b \<in> top1_S2 - D \<and>
    ...
    using hsep sorry \<comment> \<open>separation \<Rightarrow> \<exists> two points with no connecting path.\<close>
