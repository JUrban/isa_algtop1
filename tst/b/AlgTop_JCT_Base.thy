...
proof -
  ...
  have hevenly_bot: "top1_evenly_covered_on top1_S1 top1_S1_topology top1_S1 top1_S1_topology q U_bot"
    sorry \<comment> \<open>Same as U_top: Q2 = {x<0,y>0}, Q4 = {x>0,y<0}. Inverse: \<pm>(sqrt,sqrt) with mixed signs.\<close>
...
lemma squaring_map_factorization:
  ...
  sorry \<comment> \<open>Define k via Theorem 22.2 (quotient map universal property).
     q is a quotient map (covering \<Rightarrow> open surjection \<Rightarrow> quotient).
     g = q \<circ> h satisfies: q(x) = q(y) \<Rightarrow> q(h(x)) = q(h(y))
     (since q(x)=q(y) iff y=\<pm>x, and h(-x)=-h(x) gives q(h(-x))=q(h(x))).
     By Theorem_22_2, get k with k\<circ>q = q\<circ>h and k continuous.\<close>
