  ...
  proof -
    ...
    proof -
      ...
      have "?mul cid cid = cid"
        sorry \<comment> \<open>const * const \<simeq> const (left identity), so [const]*[const] = [const].\<close>
    ...
    have hmul_eq_id: "?mul ckf ckf = cid"
      sorry \<comment> \<open>From hkf2_triv: (k\<circ>f)*(k\<circ>f) \<simeq> const. So their \<pi>_1 classes are equal.\<close>
